...
/* 获取HUB描述符 */
__attribute__((aligned(4)))   const uint8_t SetupGetU2HubDescr[] =
{
HUB_GET_HUB_DESCRIPTOR,
HUB_GET_DESCRIPTOR, 0x00,
...
uint8_t AnalyzeU2HidIntEndp(uint8_t *buf, uint8_t HubPortIndex)
{
	...
	if (HubPortIndex)
	{
		memset(DevOnU2HubPort[HubPortIndex - 1].GpVar, 0,
				sizeof(DevOnU2HubPort[HubPortIndex - 1].GpVar)); //清空数组
	}
	else
	{
...
uint8_t InitRootU2Device(void)
{
	...
	if (s == ERR_SUCCESS)
	{
		...
		if (s == ERR_SUCCESS)
		{
			ThisUsb2Dev.DeviceAddress =
					((PUSB_SETUP_REQ) SetupSetUsb2Addr)->wValue; // 保存USB地址

			PRINT("GetU2CfgDescr: ");
			s = CtrlGetU2ConfigDescr();
			if (s == ERR_SUCCESS)
			{
				for (i = 0; i < ((PUSB_CFG_DESCR) U2Com_Buffer)->wTotalLength;
						i++)
				{
					PRINT("x%02X ", (uint16_t )(U2Com_Buffer[i]));
				}
				...
				if_cls =
						((PUSB_CFG_DESCR_LONG) U2Com_Buffer)->itf_descr.bInterfaceClass; // 接口类代码
			...
			return (ERR_USB_UNSUPPORT);
					...
					if (s == ERR_SUCCESS)
					{
						...
						&& ((PUSB_CFG_DESCR_LONG) U2Com_Buffer)->itf_descr.bInterfaceSubClass
								== 0x01)
				{                               // 是打印机类设备
					s = CtrlSetUsb2Config(cfg); // 设置USB设备配置
					if (s == ERR_SUCCESS)
				...
				{ // 是HUB类设备,集线器等
					...
					if (s == ERR_SUCCESS)
					{
						...
						ThisUsb2Dev.GpHUBPortNum =
								((PXUSB_HUB_DESCR) U2Com_Buffer)->bNbrPorts; // 保存HUB的端口数量
						...
						if (s == ERR_SUCCESS)
						{
							...
							{
								DevOnU2HubPort[i - 1].DeviceStatus =
...
uint8_t InitU2DevOnHub(uint8_t HubPortIndex)
{
	...
	DevOnU2HubPort[HubPortIndex - 1].DeviceVID =
			((uint16_t) ((PUSB_DEV_DESCR) U2Com_Buffer)->idVendor); //保存VID PID信息
	...
	if (dv_cls == 0x00 && if_cls == USB_DEV_CLASS_STORAGE) // 是USB存储类设备,基本上确认是U盘
	{
		...
		{
			PRINT("%02x ",
					(uint16_t )DevOnU2HubPort[HubPortIndex - 1].GpVar[i]);
		}
		PRINT("\n");
		s = CtrlSetUsb2Config(cfg); // 设置USB设备配置
		if (s == ERR_SUCCESS)
		{
			...
			&& (((PXUSB_CFG_DESCR_LONG) U2Com_Buffer)->itf_descr.bInterfaceSubClass
					<= 0x01)) // 是HID类设备,键盘/鼠标等
	{
		...
		PRINT("AnalyzeU2HidIntEndp %02x\n", (uint16_t )s);
		if_cls =
				((PXUSB_CFG_DESCR_LONG) U2Com_Buffer)->itf_descr.bInterfaceProtocol;
		s = CtrlSetUsb2Config(cfg); // 设置USB设备配置
		if (s == ERR_SUCCESS)
		{
...
uint8_t EnumAllU2HubPort(void)
{
	...
	if ((ThisUsb2Dev.DeviceStatus >= ROOT_DEV_SUCCESS)
			&& (ThisUsb2Dev.DeviceType == USB_DEV_CLASS_HUB)) // HUB枚举成功
	{
		SelectU2HubPort(0);  // 选择操作指定的ROOT-HUB端口,设置当前USB速度以及被操作设备的USB地址
...
uint16_t U2SearchTypeDevice(uint8_t type)
{
	...
	if ((ThisUsb2Dev.DeviceType == USB_DEV_CLASS_HUB)
			&& (ThisUsb2Dev.DeviceStatus >= ROOT_DEV_SUCCESS)) // 外部集线器HUB且枚举成功
	{
		for (HubPortIndex = 1; HubPortIndex <= ThisUsb2Dev.GpHUBPortNum;
				HubPortIndex++) // 搜索外部HUB的各个端口
		{
			if (DevOnU2HubPort[HubPortIndex - 1].DeviceType
					== type&& DevOnU2HubPort[HubPortIndex - 1].DeviceStatus >= ROOT_DEV_SUCCESS)
			{
