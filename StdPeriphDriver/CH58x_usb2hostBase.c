...
/*获取配置描述符*/
__attribute__((aligned(4)))   const uint8_t SetupGetU2CfgDescr[] =
{
USB_REQ_TYP_IN,
USB_GET_DESCRIPTOR, 0x00,
USB_DESCR_TYP_CONFIG, 0x00, 0x00, 0x04, 0x00 };
/*设置USB地址*/
__attribute__((aligned(4)))   const uint8_t SetupSetUsb2Addr[] =
{
USB_REQ_TYP_OUT,
USB_SET_ADDRESS,
USB_DEVICE_ADDR, 0x00, 0x00, 0x00, 0x00, 0x00 };
/*设置USB配置*/
...
/*设置USB接口配置*/
__attribute__((aligned(4)))   const uint8_t SetupSetUsb2Interface[] =
{
USB_REQ_RECIP_INTERF,
USB_SET_INTERFACE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
/*清除端点STALL*/
__attribute__((aligned(4)))   const uint8_t SetupClrU2EndpStall[] =
{
USB_REQ_TYP_OUT | USB_REQ_RECIP_ENDP,
USB_CLEAR_FEATURE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
uint8_t EnableRootU2HubPort(void)
{
	...
	{ // 有设备
		...
		{ // 尚未使能
			ThisUsb2Dev.DeviceSpeed =
					(R8_USB2_MIS_ST & RB_UMS_DM_LEVEL) ? 0 : 1;
...
uint8_t U2HostCtrlTransfer(uint8_t *DataBuf, uint8_t *RetLen)
{
	...
	if (s != ERR_SUCCESS)
		return (s);
	R8_U2H_RX_CTRL = R8_U2H_TX_CTRL = RB_UH_R_TOG | RB_UH_R_AUTO_TOG
			| RB_UH_T_TOG | RB_UH_T_AUTO_TOG; // 默认DATA1
