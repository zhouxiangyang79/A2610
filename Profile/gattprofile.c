...
{
ATT_BT_UUID_SIZE, simpleProfileServUUID };
...
static uint8_t simpleProfileChar1[SIMPLEPROFILE_CHAR1_LEN] =
{ 0 };
...
static uint8_t simpleProfileChar2[SIMPLEPROFILE_CHAR2_LEN] =
{ 0 };
...
static uint8_t simpleProfileChar3[SIMPLEPROFILE_CHAR3_LEN] =
{ 0 };
...
static uint8_t simpleProfileChar4[SIMPLEPROFILE_CHAR4_LEN] =
{ 0 };
...
static uint8_t simpleProfileChar5[SIMPLEPROFILE_CHAR5_LEN] =
{ 0 };
 ...
 */
static bStatus_t
simpleProfile_ReadAttrCB(uint16_t connHandle, gattAttribute_t *pAttr,
		uint8_t *pValue, uint16_t *pLen, uint16_t offset, uint16_t maxLen,
		uint8_t method);
static bStatus_t
simpleProfile_WriteAttrCB(uint16_t connHandle, gattAttribute_t *pAttr,
		uint8_t *pValue, uint16_t len, uint16_t offset, uint8_t method);

static void
simpleProfile_HandleConnStatusCB(uint16_t connHandle, uint8_t changeType);
...
bStatus_t simpleProfile_Notify(uint16_t connHandle, attHandleValueNoti_t *pNoti)
{
	...
	if (value & GATT_CLIENT_CFG_NOTIFY)
	{
		// Set the handle
		pNoti->handle =
				simpleProfileAttrTbl[SIMPLEPROFILE_CHAR4_VALUE_POS].handle;
...
bStatus_t simpleProfile_Notify_char1(uint16_t connHandle,
		attHandleValueNoti_t *pNoti)
{
	...
	if (value & GATT_CLIENT_CFG_NOTIFY)
	{
		// Set the handle
		pNoti->handle =
				simpleProfileAttrTbl[SIMPLEPROFILE_CHAR1_VALUE_POS].handle;
...
//			buff[6] = para2.altitude;
//			buff[7] = para2.runningmode;
//			if (para2.runningmode == 1)
//			{
//				buff[8] = para1.setlevel;
//			}
//			else
//			{
//				buff[8] = para1.settemp;
//			}
//			buff[9] = para2.supplyvoltage;
//			buff[10] = para2.casetemp;
//			buff[11] = para2.cabtemp;
//			buff[12] = buff[2] - buff[11];
			tmos_memcpy(pValue, buff, *pLen);
			break;
 ...
 * @return      none
 */
static void simpleProfile_HandleConnStatusCB(uint16_t connHandle,
		uint8_t changeType)
{
